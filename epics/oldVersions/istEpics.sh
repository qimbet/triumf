...
if [ -d "$LOCAL_DEB_REPO" ] && [ "$(ls -A "$LOCAL_DEB_REPO")" ]; then
    ...
    apt update
    apt install -y \
    libpng-dev libmotif-dev libxm4 zlib1g-dev libreadline-dev libncurses-dev libgif-dev libx11-dev libxtst-dev \
...
if true; then
    ...
    # source $EPICS_ROOT/epics_env.sh
...
# mkdir -p "$EPICS_EXTENSIONS/lib/linux-x86_64"
...
#     CONFIG_SITE="$EPICS_EXTENSIONS/configure/CONFIG_SITE"
...
#     cat > "$EPICS_EXTENSIONS/configure/CONFIG_COMMON" <<EOF
# # Minimal CONFIG_COMMON for EDM
# EOF
#     cat > "$EPICS_EXTENSIONS/configure/RELEASE" <<EOF
...
#     # -------------------------------
#     # Create symlinks to fix libX___ pathing issues across distros
#     # -------------------------------
#     REQUIRED_LIBS=(libXm.so libXt.so libX11.so libXext.so libXmu.so libXtst.so)

#     sudo mkdir -p /usr/lib64

#     for lib in "${REQUIRED_LIBS[@]}"; do
#         TARGET="/usr/lib64/${lib%.so}.a"
#         SOURCE="/usr/lib/x86_64-linux-gnu/$lib"
...
#             else
#                 echo "Warning: $SOURCE not found, EDM build may fail"
#             fi
#         fi
#     done


#     export CONFIG_SITE #duplicate -- may not be needed. Keeping to be safe
...
#     install -m 777 "$EDM_BIN" "$EPICS_EXTENSIONS/bin/$EPICS_HOST_ARCH/"
#     # sudo mkdir -p /etc/edm
#     # sudo cp "$EPICS_EXTENSIONS/src/edm/setup/edmPvObjects" /etc/edm/
