...
exec > >(tee -a "$LOGFILE") 2>&1 #print to both terminal & logfile
...
success=false #track successful edm installation
if [ -f "$SETUP_SH" ]; then
  libline='for libdir in baselib lib epicsPv locPv calcPv util choiceButton pnglib diamondlib giflib videowidget'
  # change line 53
  sed -i "53c${libline}" "$SETUP_SH" &&

  # delete line 79
  sed -i '79d' "$SETUP_SH" &&

  # insert new lines (use sed with escaped backslashes)
  sed -i "81i\\    \$EDM -add \$EDMBASE/pnglib/O.\$ODIR/lib57d79238-2924-420b-ba67-dfbecdf03fcd.so" "$SETUP_SH" &&
  sed -i "82i\\    \$EDM -add \$EDMBASE/diamondlib/O.\$ODIR/libEdmDiamond.so" "$SETUP_SH" &&
  sed -i "83i\\    \$EDM -add \$EDMBASE/giflib/O.\$ODIR/libcf322683-513e-4570-a44b-7cdd7cae0de5.so" "$SETUP_SH" &&
  sed -i "84i\\    \$EDM -add \$EDMBASE/videowidget/O.\$ODIR/libTwoDProfileMonitor.so" "$SETUP_SH" &&
